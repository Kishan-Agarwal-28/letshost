...
const TemplatesPage = ({ templates, handleDeleteTemplate,handleApplyTemplate}: { templates: Template[] ,handleDeleteTemplate: (id: string) => void,handleApplyTemplate:(id:string)=>void }) => {
  ...
  const getImageStyles  = (options: AllOptions) => {
    ...
    return {
      ...
      `.trim(),
      border: borders.border.width > 0 ? `${borders.border.width}px solid ${borders.border.color}` : 'none',
      padding: borders.padding.size > 0 ? `${borders.padding.size}px` : '0',
      backgroundColor: borders.padding.size > 0 ? borders.padding.color : 'transparent',
      borderRadius: '8px',
      ...
      <div className="min-h-screen  p-4">
        <div className="max-w-7xl mx-auto">
          <div className="text-center py-20">
            ...
            <h2 className="text-3xl font-bold text-gray-900 mb-4">No Templates Found</h2>
            <p className="text-gray-600 max-w-md mx-auto">
              You haven't created any templates yet. Templates will appear here once you save them.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6">
          {templates.map((template) => (
            ...
            <Card key={template.id} className="group hover:shadow-xl transition-all duration-300 overflow-hidden border-0 shadow-lg backdrop-blur-sm cursor-pointer" onClick={()=>handleApplyTemplate(template.id)}>
              ...
              <CardContent className="pt-0">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                    {template.options.resize.width && template.options.resize.height && (
                  <div className="bg-gray-400 p-2 rounded">

                    <span className="text-gray-500">Size:</span>
                    <div className="font-medium">{template.options.resize.width}x{template.options.resize.height}</div>
                  </div>
                    )}
                    {template.options.output.format && (
                  <div className="bg-gray-400 p-2 rounded">
                    <span className="text-gray-500">Format:</span>
                    <div className="font-medium uppercase">{template.options.output.format}</div>
                  </div>
                    )}
                </div>
                
                {(template.options.colors.effect !== 'none' || template.options.filters.blur > 0) && (
              ...
              <TooltipContent className="bg-muted-foreground  text-balance"  tip={false}>
