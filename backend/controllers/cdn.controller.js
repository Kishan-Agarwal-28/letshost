...
const registerCDN = asyncHandler(async (req, res) => {
  ...
  const types = mime.lookup(
    Array.isArray(req.files)
      ? req.files[0].filename
      : req.body.fileMetaData.filename
  );
  console.log("types", types);
  const contentType = types.toString()?.toLowerCase()?.includes("javascript")
    ? "js"
    : types.toString()?.toLowerCase()?.includes("css")
      ? "css"
      : types.toString()?.toLowerCase()?.includes("image")
        ? "image"
        : types.toString()?.toLowerCase()?.includes("video")
          ? "video"
          : types.toString()?.toLowerCase()?.includes("mp4")
            ? "video"
            : undefined;
  if (!contentType) {
  ...
  switch (contentType) {
    case "js":
      if (user.totalJsCssSize + req.files[0].size > pricing.cdnCSSJSlimit) {
        throw new apiError(
          400,
          "File size exceeds the limit upgrade to pro tier"
        );
      }

      url = await uploadToCDN(
        userID,
        currentVersion,
        prevVersion,
        cdnProjectID
    ...
    case "video":
      ...
      const uploadResultVideo = await uploadMediaVideoToCDN(
        userID,
        currentVersion,
        prevVersion,
        cdnProjectID
      ...
      return res
        .status(200)
        .json(
          new apiResponse(200, uploadResultVideo, "File uploaded successfully")
        );
    default:
      throw new apiError(400, "Unsupported file type");
  }
...
const updateCDN = asyncHandler(async (req, res) => {
  ...
  switch (contentType) {
    case "js":
      if (
        user.cdnCSSJSlimit + req.files[0].size - prevCdn.size >
        pricing.cdnCSSJSlimit
      ) {
        throw new apiError(
          400,
          "File size exceeds the limit upgrade to pro tier"
      ...
      user.totalJsCssSize =
        user.totalJsCssSize - prevCdn.size + req.files[0].size;
      await user.save();
    ...
    case "image":
      if (
        user.totalMediaSize + req.files[0].size - prevCdn.size >
        pricing.cdnMedialimit
      ) {
        throw new apiError(
          400,
      ...
      user.totalMediaSize =
        user.totalMediaSize - prevCdn.size + req.files[0].size;
      await user.save();
      const pathname = new URL(uploadResult.url).pathname;
      const ImageUrl = pathname.replace("/testifywebdev/", "");
      prevCdn.filename = req.files[0].filename;
    ...
    case "video":
      ...
      if (
        user.cdnMedialimit + req.body.fileMetaData.size - prevCdn.size >
        pricing.cdnMedialimit
      ) {
        throw new apiError(
          400,
      ...
      user.totalMediaSize =
        user.totalMediaSize - prevCdn.size + req.body.fileMetaData.size;
      await user.save();
...
const deleteCDN = asyncHandler(async (req, res) => {
  ...
  if (prevCdn.bucketAssigned === "cdn") {
    await deleteFromCDN(
      req.user._id,
      prevCdn.cdnProjectID,
      prevCdn.currentVersion,
      prevCdn.fileType
    );
    ...
    const publicId = prevCdn.relativePath.includes(
      "http://res.cloudinary.com/testifywebdev/"
    )
      ? prevCdn.relativePath.split("/").pop().split(".")[0]
      : prevCdn.relativePath;

    const folder =
      prevCdn.fileType === "image"
        ? "img"
        : prevCdn.fileType === "video"
          ? "video"
          : "";

    await deleteMediaFromCDN(
      req.user._id,
      prevCdn.currentVersion,
      folder,
      prevCdn.cdnProjectID
...
const confirmVideoUpload = asyncHandler(async (req, res) => {
  if (
    req.body.notification_type === "upload" &&
