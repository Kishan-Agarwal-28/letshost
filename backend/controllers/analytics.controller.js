...
const getAnalytics = asyncHandler(async (req, res) => {
  try {
    ...
    if (user.totalJsCssSize !== totalJsCssSize || user.totalMediaSize !== totalMediaSize) {
    ...
    const subdomainUsage = (formattedSubdomains.length / userData.SDLimit) * 100;
    ...
    const jssCssUsage = (userData.totalJsCssSize / userData.cdnCSSJSlimit) * 100;
...
const generateMonthlyUsage = async (userId) => {
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    const storageUsed = filesInMonth.reduce((total, file) => total + file.size, 0);
