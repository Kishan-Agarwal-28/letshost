...
import {searchSimilarImages, findSimilarImagesById, advancedImageSearch, getRandomImages} from "../services/quad.service.js";
...
const deleteImage = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        return res.status(200)
            .json(new apiResponse(200, null, "Image deleted successfully. 2 credits deducted."));
    ...
    const images = await Image.aggregate([
        ...
        },
        {
            $addFields: {
                ...
                isLikedByUser: req.user && req.user._id ? {
                    $in: [new mongoose.Types.ObjectId(req.user._id), "$likes.likedBy"]
                } : false,
                // Fixed: Check if current user's ID exists in the saves array  
                isSavedByUser: req.user && req.user._id ? {
                    $in: [new mongoose.Types.ObjectId(req.user._id), "$saves.savedBy"]
                } : false
            }
        },
        {
...
const getTotalImagesCount=asyncHandler(async (req,res)=>{
    ...
    return res.status(200)
    .json(new apiResponse(200,count,"Total images count fetched successfully"));
})
...
const getSimilarImages = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(
    new apiResponse(200, results, "Similar images found successfully")
  );
});
...
const discoverImages = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(
    new apiResponse(200, { images: results }, "Random images retrieved successfully")
  );
});

export {addToGallery,likeImage,saveImage,deleteImage,getImagesOfUser,getGallery,getTotalImagesCount,searchImages,getSimilarImages,advancedSearch,discoverImages,downloadImage};