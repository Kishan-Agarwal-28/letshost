...
const enhancePrompt = (userPrompt, options = {}) => {
  ...
  const aspectRatio = options.aspectRatio ? `aspect ratio ${options.aspectRatio}` : '';
...
const generateImage = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        let metadata = {
            ...
            description: req.body.prompt.substring(0, 100) + (req.body.prompt.length > 100 ? "..." : ""),
            ...
            if (jsonMatch) {
                try {
                    ...
                    metadata = {
                        ...
                        tags: Array.isArray(extractedMetadata.tags) ? extractedMetadata.tags : metadata.tags
                    };
                } catch (parseError) {
  ...
  tags: metadata.tags
        ...
        throw new apiError(error.statusCode || 500, `Image generation failed: ${error.message}`);
    }
});
