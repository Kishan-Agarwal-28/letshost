 ...
 */
function sanitizeValue(value, depth = 0, maxDepth = 10, visited = new WeakSet()) {
    ...
    try {
      const sanitized = value.map((item, index) => {
        try {
          ...
          console.error(`Array item sanitization error at index ${index}:`, error);
          return null;
        }
      });
      ...
      for (const key of keys) {
        try {
          ...
          if (typeof sanitizedKey === 'string' && sanitizedKey.length > 0) {
            // Sanitize the value
            sanitized[sanitizedKey] = sanitizeValue(value[key], depth + 1, maxDepth, visited);
          }
        } catch (error) {
          console.error(`Object property sanitization error for key "${key}":`, error);
