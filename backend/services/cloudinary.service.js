...
const listMediaObjectsOnCDN = async (version, folder, userID, cdnID) => {
  try {
    ...
    if (errorMessage.includes('not found') || errorMessage.includes('does not exist') || error?.http_code === 404) {
      console.warn(`CDN folder not found: __cdn/${userID}/${folder}/${cdnID}/${version}`);
      return [];
    }
...
const deleteMediaFromCDN = async (userID, version, folder, cdnID, public_id) => {
  try {
    if (!public_id) {
      const publicIds = await listMediaObjectsOnCDN(version, folder, userID.toString(), cdnID);
      
      // If no public IDs found, nothing to delete
      if (!publicIds || publicIds.length === 0) {
        console.warn(`No media objects found for CDN folder: __cdn/${userID}/${folder}/${cdnID}/${version}`);
        return { deleted: [], not_found: [] };
      }
      
      const fileType = folder === "img" ? "image" : folder === "video" ? "video" : "auto";
      ...
      const resourceType = folder === "img" ? "image" : folder === "video" ? "video" : "auto";
    ...
    if (errorMessage.includes('not found') || errorMessage.includes('does not exist') || error?.http_code === 404) {
      console.warn(`CDN resource not found: __cdn/${userID}/${folder}/${cdnID}/${version}`);
...
const uploadMediaToCDN = async (userID, currentVersion, previousVersion, cdnID) => {
  ...
  try {
    ...
    const folder = contentType?.toString().toLowerCase()?.includes("image") ? "img" : 
                  contentType?.toString().toLowerCase()?.includes("video") ? "video" : 
                  contentType?.toString().toLowerCase()?.includes("mp4") ? "video" : undefined;
    ...
    throw new Error("Failed to upload to CDN: " + (error?.message || 'Unknown error'));
  }
};

const uploadMediaVideoToCDN = async (userID, currentVersion, previousVersion, cdnID) => {
...
const deleteEmptyFolders = async (userID, folder, cdnProjectID) => {
  try {
    ...
    if (errorMessage.includes('not found') || errorMessage.includes('does not exist') || error?.http_code === 404) {
      console.warn(`Folder not found (already deleted?): __cdn/${userID}/${folder}/${cdnProjectID}`);
