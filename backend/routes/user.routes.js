...
router.route("/resendVerificationToken").get(
  logger,
  rateLimit({
    windowMs: 60 * 1000 * 60,
    max: 3,
    message: {
      message:
        "Too many attempts Please try again after an hour You can only make 3 attempts per hour",
    },
...
router
  .route("/changeEmail")
  .get(logger, verifyJWT, verifiedMiddleware, changeEmail);
router
  .route("/updateEmail")
  .post(logger, verifyJWT, verifiedMiddleware, updateEmail);
router
  .route("/changeUserName")
  .post(logger, verifyJWT, verifiedMiddleware, changeUserName);
router
  .route("/sendUpdatePasswordEmail")
  .post(logger, verifyJWT, verifiedMiddleware, sendUpdatePasswordEmail);
router
  .route("/updatePassword")
  .post(logger, verifyJWT, verifiedMiddleware, updatePassword);
router
  .route("/deleteUser")
  .delete(logger, verifyJWT, verifiedMiddleware, deleteUser);
router
  .route("/updateAvatar")
  .patch(
    logger,
...
router
  .route("/verify2FA")
  .post(logger, verifyJWT, verifiedMiddleware, verify2FA);
...
router
  .route("/disable2FA")
  .post(logger, verifyJWT, verifiedMiddleware, disable2FA);
router
  .route("/payment")
